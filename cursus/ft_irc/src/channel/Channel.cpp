...
std::string							Channel::getChannelPassword() const	{ return (_channel_password); }
int									Channel::getCapacityLimit() const	{ return (_capacity_limit); }
 ...
 * @param clientName 
 */
void	Channel::removeClientFromChannel(const std::string &clientName)
{
...
bool	Channel::isBanned(const std::string &banned_name)
{
	std::vector<std::string>::iterator user;
...
bool	Channel::isVoiced(const std::string &voiced_name)
{
	std::vector<std::string>::iterator user;
