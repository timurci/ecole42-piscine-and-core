...
/*   Updated: 2022/04/04 15:11:09 by tcakmako         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static long	loc_set(char *s, int mem, t_shape *shape)
{
...
static char	*get_type(char *tn, va_list ap, t_shape *shape)
{
	if (!ft_tkncmp(tn, "p"))
		...
		return (conv_hex(va_arg(ap, unsigned int), *tn, shape));
	else if (!ft_tkncmp(tn, "%"))
		return (conv_str("%", shape));
	return (NULL);
}
...
long	pformat(const char *input, int mem, int *print_len, va_list ap)
{
	...
	if (!typeholder)
		return (-1);
	*print_len += write(1, typeholder, shape.w1);
	free(typeholder);
	return (end + 1);
}
