...
static void	recondition(t_shape *shape, char *type, size_t ssize)
{
	...
	if (*type == 'p' && (size_t) shape->w1 < ssize + 2)
		shape->w1 += 2;
	if (shape->flags & (8 | 32))
		*type = '0';
	else
...
static char	*reshape(char *s, t_shape *sh, char type, size_t slen)
{
	...
	if (sh->flags & 64 && !(sh->flags & 4))
		...
		fill(&news[(sh->flags & 128) / 128 * 2], type, sh->w0);
	else
		fill(news, type, sh->w1);
	if ((sh->flags & 128) && (sh->flags & (4 | 8) || (sh->flags & 96) == 32))
		ft_memcpy(news, "0x", 2);
	else if (sh->flags & 128)
		ft_memcpy(&news[sh->w1 - sh->w0 - 2], "0x", 2);
	if ((sh->flags & (4 | 32 | 64)) == 36)
		ft_memcpy(&news[sh->w1 - slen], s, slen);
	else if ((sh->flags & (4 | 32)) == 36 || sh->flags & 4)
		ft_memcpy(&news[(sh->flags & 128) / 128 * 2 + sh->w0 - slen], s, slen);
	else
		ft_memcpy(&news[sh->w1 - slen], s, slen);
	free(s);
	return (news);
